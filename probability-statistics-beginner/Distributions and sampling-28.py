...
df = income[income["pop_over_25"] > 500000]
lowest_income_high_pop_county = df["county"][df["median_income"].idxmin()]
...
random.seed(1)
shopping_sample = random.sample(shopping, 2)
...
random.seed(1)
med_sample = probability_of_one(300, 100)
plt.hist(med_sample, 20)
plt.ylim(0,70)
plt.xlim(0,0.4)
plt.show()

random.seed(1)
high_sample = probability_of_one(300, 1000)
plt.hist(high_sample, 20)
plt.ylim(0,70)
plt.xlim(0,0.4)
plt.show()
...
for  i in mean_ratios:
    if i >= 0.675:
        num += 1
significance_value = num / len(mean_ratios)