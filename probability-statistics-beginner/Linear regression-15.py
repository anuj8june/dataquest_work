...
num = cov(wine_quality["quality"],wine_quality["density"])
den = wine_quality["density"].var()
slope_density = num / den

slope_density1 = cov(wine_quality["density"], wine_quality["quality"])[0, 1] / wine_quality["density"].var()
...
xval = wine_quality["density"]
yval = wine_quality["quality"]
slope = calc_slope(xval,yval)
y = calc_intercept(xval,yval,slope)
...
predicted_quality = wine_quality["density"].apply(cal_y)
...
print(slope)
print(intercept)
ypre = slope * wine_quality["density"] + intercept
rss = sum((wine_quality["quality"] - ypre) ** 2)
