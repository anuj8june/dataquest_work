...
point_variance = sum([(pt-nba_stats["pts"].mean())**2 for pt in nba_stats["pts"]])/len(nba_stats["pts"])
...
def sd_cal(data):
    return (sum([(pt-data.mean())**2 for pt in data])/len(data)) ** 0.5
    
mp_dev = sd_cal(nba_stats["mp"])
ast_dev = sd_cal(nba_stats["ast"])
...
wing_std_dev = []
for val in wing_lengths:
    wing_std_dev.append((val - np.mean(wing_lengths)) / np.std(wing_lengths))
    

count1 = 0
for val in wing_std_dev:
    if val > -1 and val <1:
        count1 += 1 
...
within_one_percentage = count1/len(wing_lengths)
...
def cov_cal(xvals,yvals):
    xmean = sum(xvals) / len(xvals)
    ymean = sum(yvals) / len(yvals)
    xdiff = [i - xmean for i in xvals]
    ydiff = [i - xmean for i in yvals]
    prod = 0
    for k,val in enumerate(xdiff):
        prod = prod + val * ydiff[k]
    
    return prod / len(xdiff)

cov_stl_pf = cov_cal(nba_stats["stl"],nba_stats["pf"])
cov_fta_pts = cov_cal(nba_stats["fta"],nba_stats["pts"])
...
r_fta_blk = cov(nba_stats["fta"], nba_stats["blk"])
r_ast_stl = cov(nba_stats["ast"], nba_stats["stl"])