...
mean_car_speed = mean_maker(car_speeds)
mean_earthquake_intensities = mean_maker(earthquake_intensities)
...
degrees_zero = []
for val in fahrenheit_degrees:
    value = val + 459.67
    degrees_zero.append(value)
...
average_smoking = sum/len(survey_responses)
...
for i,val in enumerate(gender):
    if val == "male":
        male_sum = male_sum + savings[i]
        cm = cm + 1
    if val == "female":
        female_sum = female_sum + savings[i]
        cf = cf + 1

male_savings = male_sum / cm
female_savings = female_sum / cf
